    ...
    function getFollowingList(address user) view public returns(uint[] memory ids) {
    ...
    function followUser(uint id) userExists  public {
        ...
        users[id1].following++;
        followingList[msg.sender].push(id);
        emit followed(users[id].followers, users[id1].following);
    }

    function unfollowUser(uint id) userExists  public {
        address toBeunFollowed1= users[id].pkey;
        require(msg.sender != toBeunFollowed1);
        users[id].followers--;

        uint id1 = addressToId[msg.sender];
        users[id1].following--;

        uint[] memory temp = getFollowingList(msg.sender);
        for(uint i=0; i<temp.length; i++){
            if(temp[i] == id){
                followingList[msg.sender][i] = MAX_INT;
            }
        }
        emit followed(users[id].followers, users[id1].following);
    }
